<!DOCTYPE html>
<html lang="ja">
<head>
     <meta charset="UTF-8">
    <title>mission_3-5</title>
</head>
    <body>
     <?php
      $filename='mission_3-5.txt';
      
       //削除機能
       if(!empty($_POST["num"]) && !empty($_POST["pass2"])) {
           $num=$_POST["num"];
           $pass2=$_POST["pass2"];
     
            $lines = file($filename,FILE_IGNORE_NEW_LINES); 
            $fp=fopen($filename, 'w'); 
            foreach( $lines as $line ){
             $delete=explode("<>", $line);    
             $POSTnumber=$delete[0];
             $POSTpass=$delete[4];
                if($POSTnumber !== $num && $POSTpass=$pass2){
                    fwrite($fp, $line .PHP_EOL);
                }
            }
            fclose($fp);
            
           
        }
        
             //フォームが空じゃないかつ編集番号が無いなら新規書き込み、編集番号があるなら編集
              
                if(!empty($_POST["str"]) && !empty($_POST["name"]) && !empty($_POST["pass"])){ 
                  $name=$_POST['name'];
                  $str=$_POST['str'];
                  $pass=$_POST["pass"];
                  $date=date("Y/m/d H:i:s");
                  
                  //編集フォームに書きこみがなかった場合(新規書き込み機能)
                  if(empty($_POST["editNO2"])){
                      
                    //ファイルに書き込まれた回数を取得(1 or それ以外)
                    if (file_exists($filename)) {
                    $num = count(file($filename))+1;
                     } else { $num = 1; }  
                    $newdata = $num.'<>'. $name.'<>'. $str.'<>'. $date .'<>'.$pass.'<>';
                      $fp=fopen($filename, 'a'); 
                      fwrite($fp, $newdata .PHP_EOL);
                      fclose($fp);
                      
                      
                   }
                }
                   //編集フォームに書きこみがあった場合、その番号の書き込みを取得
                   if(!empty($_POST["editNO"]) &&!empty($_POST["pass3"])){
                        $editNO=$_POST["editNO"];
                        $pass3=$_POST["pass3"];
                        $lines = file($filename,FILE_IGNORE_NEW_LINES);  
                        foreach( $lines as $line ){
                        $comment=explode("<>", $line);  
                    //編集フォームの番号と、ファイルに書きこまれた数字が一致する時
                        if($comment[0] == $editNO && $comment[4]==$pass3){
                            $editname  =$comment[1];
                            $editstr   =$comment[2];
                        }
                        }
                   }
                   //編集したい内容が返ってきて、editNO2に数字がある時
                    elseif(!empty($_POST["editNO2"])){
                      if(!empty($_POST["str"]) && !empty($_POST["name"]) && !empty($_POST["pass"]) ){ 
                          
                          $editNO2=$_POST["editNO2"];
                          $name=$_POST['name'];
                          $str=$_POST['str'];
                          $pass=$_POST["pass"];
                          $date=date("Y/m/d H:i:s");
                          $lines = file($filename,FILE_IGNORE_NEW_LINES);  
                          $fp=fopen($filename, 'w'); 
                          foreach( $lines as $line ){
                          $comment2=explode("<>", $line);  
                          //編集フォームの番号と、ファイルに書きこまれた数字が一致する時
                          if($comment2[0] == $editNO2 &&$comment2[4]=$pass){
                              
  
                            $editdata=  $editNO2.'<>'. $name.'<>'. $str.'<>'. $date.'<>'.$pass;
                            
                            fwrite($fp, $editdata .PHP_EOL);  
                           
                           }  
                           else{
                               fwrite($fp, $line .PHP_EOL); }
                           }
                        fclose($fp);
                        }
                          
                          
                     }
 
       ?>
        <form action="" method="post" >　
        <table>
        <input type="text" name="name"value="<?php if(isset($editname)) {echo $editname;} ?>"placeholder="名前">
        <input type="text" name="str" value="<?php if(isset($editstr)) {echo $editstr;} ?>"placeholder="コメント">　<!-- value･･･初期表示の文字を指定 -->
        <input type="text" name="pass" placeholder="パスワード">
        <input type="hidden" name="editNO2" value="<?php if(isset($editNO)) {echo $editNO;} ?>">
        <input type="submit" name="submit">
        </table>
       </form>
       
       <form action="" method="post" >
        <input type="number" name="num" >
        <input type="text" name="pass2" placeholder="パスワード" >
        <input type="submit" name="submit" value="削除">
        
         </form>
          <form action="" method="post" >
        <input type="number" name="editNO" >
         <input type="text" name="pass3" placeholder="パスワード" >
        <input type="submit" name="submit" value="編集">
       
         </form>
         
         <?php
         if (file_exists($filename)){
         $lines = file($filename,FILE_IGNORE_NEW_LINES);
                      foreach( $lines as $line ){
                      $line=explode("<>", $line); 
                      $line = str_replace('<>', ' ', $line);
                      echo $line[0]." ". $line[1]." ". $line[2]." ". $line[3]."<br>" ; }
         }
         ?>
    </body>
</html>